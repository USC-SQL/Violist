...
The configuration file uses the (https://commons.apache.org/proper/commons-configuration/)[Apache Commons-IO Configuration format].  The options are described below. See (an example in the repo)[example/config.properties]  Developers can add new options to the configuration file to provide additional properties needed by their custom interpreters.  Interactions with the configuration file are encapsulated in the (src/main/java/usc/sq/violist/util/ViolistConfiguration.java)[ViolistConfiguration] class.
